...
describe('resenasRepository', () => {
  it('list calls select(*)', async () => {
    await resenasRepository.list();
    expect(mockTable.select).toHaveBeenCalledWith('*');
  });

  it('findById calls select eq maybeSingle', async () => {
    await resenasRepository.findById('123');
    expect(mockTable.select).toHaveBeenCalledWith('*');
    expect(mockTable.eq).toHaveBeenCalledWith('id_resena', '123');
    expect(mockTable.maybeSingle).toHaveBeenCalled();
  });

  it('create calls insert select single', async () => {
    const payload = { id_resena: '123' };
    await resenasRepository.create(payload as any);
  ...
  it('update calls update eq select single', async () => {
    const payload = { id_resena: '123' };
    await resenasRepository.update('123', payload as any);
  ...
  it('remove calls delete eq', async () => {
    await resenasRepository.remove('123');
