...
describe('pagosRepository', () => {
  it('list calls select(*)', async () => {
    await pagosRepository.list();
    expect(mockTable.select).toHaveBeenCalledWith('*');
  });

  it('findById calls select eq maybeSingle', async () => {
    await pagosRepository.findById('123');
    expect(mockTable.select).toHaveBeenCalledWith('*');
    expect(mockTable.eq).toHaveBeenCalledWith('id_pago', '123');
    expect(mockTable.maybeSingle).toHaveBeenCalled();
  });

  it('create calls insert select single', async () => {
    const payload = { id_pago: '123' };
    await pagosRepository.create(payload as any);
  ...
  it('update calls update eq select single', async () => {
    const payload = { id_pago: '123' };
    await pagosRepository.update('123', payload as any);
  ...
  it('remove calls delete eq', async () => {
    await pagosRepository.remove('123');
