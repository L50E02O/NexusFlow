...
import { detalleCarritosRepository } from './detalle-carritos.repository';
...
describe('detalleCarritosRepository', () => {
  it('list calls select(*)', async () => {
    await detalleCarritosRepository.list();
    expect(mockTable.select).toHaveBeenCalledWith('*');
  });

  it('findById calls select eq maybeSingle', async () => {
    await detalleCarritosRepository.findById('123');
    expect(mockTable.select).toHaveBeenCalledWith('*');
    expect(mockTable.eq).toHaveBeenCalledWith('id_detalle', '123');
    expect(mockTable.maybeSingle).toHaveBeenCalled();
  });

  it('create calls insert select single', async () => {
    const payload = { id_detalle: '123' };
    await detalleCarritosRepository.create(payload as any);
  ...
  it('update calls update eq select single', async () => {
    const payload = { id_detalle: '123' };
    await detalleCarritosRepository.update('123', payload as any);
  ...
  it('remove calls delete eq', async () => {
    await detalleCarritosRepository.remove('123');
