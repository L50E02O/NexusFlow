...
describe('recomendacionRepository', () => {
  it('list calls select(*)', async () => {
    await recomendacionRepository.list();
    expect(mockTable.select).toHaveBeenCalledWith('*');
  });

  it('findById calls select eq maybeSingle', async () => {
    await recomendacionRepository.findById('123');
    expect(mockTable.select).toHaveBeenCalledWith('*');
    expect(mockTable.eq).toHaveBeenCalledWith('id_recomendacion', '123');
    expect(mockTable.maybeSingle).toHaveBeenCalled();
  });

  it('create calls insert select single', async () => {
    const payload = { id_recomendacion: '123' };
    await recomendacionRepository.create(payload as any);
  ...
  it('update calls update eq select single', async () => {
    const payload = { id_recomendacion: '123' };
    await recomendacionRepository.update('123', payload as any);
  ...
  it('remove calls delete eq', async () => {
    await recomendacionRepository.remove('123');
