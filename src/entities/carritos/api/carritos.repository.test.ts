...
import { carritosRepository } from './carritos.repository';
...
describe('carritosRepository', () => {
  it('list calls select(*)', async () => {
    await carritosRepository.list();
    expect(mockTable.select).toHaveBeenCalledWith('*');
  });

  it('findById calls select eq maybeSingle', async () => {
    await carritosRepository.findById('123');
    expect(mockTable.select).toHaveBeenCalledWith('*');
    expect(mockTable.eq).toHaveBeenCalledWith('id_carrito', '123');
    expect(mockTable.maybeSingle).toHaveBeenCalled();
  });

  it('create calls insert select single', async () => {
    const payload = { id_carrito: '123' };
    await carritosRepository.create(payload as any);
  ...
  it('update calls update eq select single', async () => {
    const payload = { id_carrito: '123' };
    await carritosRepository.update('123', payload as any);
  ...
  it('remove calls delete eq', async () => {
    await carritosRepository.remove('123');
